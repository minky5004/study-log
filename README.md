...
**해시는 작은따옴표를 벗기지 않는다.** 안에 든 `$` 를 컴포즈가 변수로 읽어 값을 잘라내는데,
잘린 값도 `$2` 로 시작해 부팅 검사를 통과하므로 앱은 뜨고 로그인만 영영 실패한다. `.env.example`
을 복사만 하고 해시를 채우지 않았을 때도 결말이 같다 — 자리표시자 역시 `$2` 로 시작한다.

조회 화면은 로그인 없이 열리고, 기록 작성·수정·삭제와 함께 **내보내기·가져오기가 로그인을
요구한다.** 둘은 GET 이지만 한 요청이 DB 전량을 흘려보내거나 받는 쪽이라 화면 단위 조회와 다르게 봤다.
...
| 분야는 최초 표기를 유지하고 태그는 소문자로 통일 | 분야는 화면에 이름 그대로 나가고 태그는 그렇지 않다. 대소문자만 다른 분야가 통계를 가르지 않도록 중복 판정은 정규화 컬럼 `name_key` 의 unique 제약 — `lower(name)` 함수 인덱스로 대신하지 않는 것은 키 파생이 소문자화만이 아니라 공백 축약을 함께 하기 때문이다. 문서에서 붙여넣은 전각 공백이 든 분야가 타이핑한 것과 다른 키로 갈라지면 통계가 둘로 쪼개지는데, 이 프로젝트에는 합칠 분야 관리 화면이 없다 |
...
| 스키마는 Flyway 가 만들고 하이버네이트는 대조만 | `ddl-auto=update` 로 두었을 때 실제로 물렸다 — 태그 순서 컬럼을 더하는 DDL 이 실패했는데 경고 한 줄만 남기고 부팅에 성공해, 목록이 500 인 채로 떠 있었다. `validate` 는 같은 상황에서 아예 뜨지 않는다. 마이그레이션이 PostgreSQL 방언이라 로컬 H2 에서는 끄고, 대신 리포지토리 테스트가 운영과 같은 배선으로 돈다 |
